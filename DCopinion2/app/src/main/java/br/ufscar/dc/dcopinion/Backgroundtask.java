...
public class Backgroundtask  extends AsyncTask<String,Void,ArrayList<String>>{
    ...
    protected ArrayList<String> doInBackground(String... params) {
        ...
        if(method.equals("cadastrar")) {
            texto_url = "http://200.18.97.50/webapp/cadastrar.php";
            email = params[1];
            senha = params[2];
            categoria = params[3];
        }
        else if(method.equals("login")){
            texto_url = "http://200.18.97.50/webapp/login.php";
            email = params[1];
            senha = params[2];
        }
        else if(method.equals("noticias")){
            texto_url = "http://200.18.97.50/webapp/noticias.php";
            id = params[1];
        }
        else if(method.equals("questoes_responder")){
            texto_url = "http://200.18.97.50/webapp/questoes_responder.php";
            id = params[1];
        }
        else if(method.equals("envia_resposta")){
            texto_url = "http://200.18.97.50/webapp/envia_resposta.php";
            id_questao = params[1];
            id_cliente = params[2];
